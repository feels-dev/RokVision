...
namespace RoK.Ocr.Application.Magnifier;

public class TheMagnifier
{
    ...
    public TheMagnifier(IOcrService ocrService, IImageStorage storage)
    {
    ...
    public async Task<List<OcrBlock>> HuntForField(string imagePath, AnalyzedBlock anchor, string fieldType)
    {
        ...
        Console.WriteLine($"\n[MAGNIFIER] --- Starting Hunt for: {fieldType} ---");
        ...
        foreach (var strategy in strategies)
        {
            ...
            try
            {
                Console.WriteLine($"[MAGNIFIER] Trying strategy: {strategy.Name}");
                ...
                {
                    Console.WriteLine("[MAGNIFIER] Failure: Invalid crop region.");
                    continue;
                }
                ...
                if (result.Blocks.Count > 0)
                {
                    ...
                    Console.WriteLine($"[MAGNIFIER] OCR Read: '{readText}'");

                    if (result.Blocks.Count > 0 && readText.Length >= 3)
                    {
                        isSuccess = true;
                        Console.WriteLine("[MAGNIFIER] >> MATCH! Valid text found.");
                    }
                }
                else
                {
                    Console.WriteLine($"[MAGNIFIER] OCR Failure");
                }
                ...
                if (EnableDebugMode)
                {
                    ...
                    Console.WriteLine($"[MAGNIFIER] Debug saved: {debugFilename}");
                }
        ...
        Console.WriteLine("[MAGNIFIER] --- End of Hunt (Unsuccessful) ---\n");
