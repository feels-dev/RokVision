...
using RoK.Ocr.Application.Features.Governor.Orchestrator; // Namespace updated expectation
...
{
    // Renamed from OcrOrchestrator to GovernorOrchestrator to match the new structure
    ...
    public GovernorController(
        GovernorOrchestrator orchestrator,
    ...
    public async Task<IActionResult> Analyze([FromForm] GovernorUploadRequest request)
    {
        ...
        if (request.Image == null || request.Image.Length == 0)
            return BadRequest(new GovernorApiResponse { Success = false, Message = "No image sent." });
        ...
        try
        {
            ...
            {
                return Ok(new GovernorApiResponse
                {
                    Success = false,
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new GovernorApiResponse { Success = false, Message = $"Internal error: {ex.Message}" });
        }
    }
}